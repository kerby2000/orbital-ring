...
def test_reference_closed_form_values():
    ...
    assert result.magnetic_turning_angle_rad == pytest.approx(3.75, abs=0.02)
    assert result.magnetic_curvature_radius_m / 1000.0 == pytest.approx(14.7, abs=0.1)
    assert result.total_guide_length_m / 1000.0 == pytest.approx(55.0, abs=1.0)
    assert result.guide_length_per_node_m == pytest.approx(
        result.total_guide_length_m / 96
    )
    assert result.approximation == "large-N L0 scaling"
